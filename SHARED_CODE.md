...
4. **Run PHPUnit before pushing.** `safe_file_path`, `atomic_write_json`, `temp_code_store`, and `temp_code_retrieve_and_delete` have automated tests under `tests/`. Run `composer test` locally — CI blocks red PRs and also enforces a `## Test Impact` declaration on PRs that touch these files. If your change touches a function that isn't tested yet, add tests in the same PR. See **[TESTING.md](TESTING.md)** for the full operational details, template, and priority list of untested functions.
