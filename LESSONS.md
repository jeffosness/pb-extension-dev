...
**Why we didn't catch it earlier:** The ADVERSARIAL_REVIEW_PLAYBOOK's own caveat calls this out — same-family Claude subagents *"share training-level blind spots, so their agreement is NOT the strong statistical independence that cross-family review would give."* We'd been running 2+ Claude reviewers per Tier-2 PR across the whole #228 arc (phases 1, 2, 3 all shipped without Codex). Codex was only pulled in for phase 3 (green — nothing to catch) and phase 4. Two rounds of Claude review across 82 migrated sites converged on the same blind spot: none of them modeled PHP's `+` vs `array_merge()` semantics as a class of failure worth scanning for.

**Process change:** Update the [ADVERSARIAL_REVIEW_PLAYBOOK](ADVERSARIAL_REVIEW_PLAYBOOK.md) — for Tier-2 PRs, run at least one cross-family review pass (Codex has the built-in `review` subcommand and takes ~2 minutes) alongside the Claude hostile reviewers. Same-family agreement is a signal but not a proof; cross-family agreement raises the confidence bar meaningfully. This matches CLAUDE.md's existing Tier-2 requirement of "spawn multiple parallel hostile reviewers with diverse lenses" — the missing piece was "diverse lenses AND diverse model families." Not adding a CI enforcement (reviewer choice is a judgment call, not a mechanical gate) — this lives in the playbook as guidance. Also filed #242 for the specific reserved-key collision hardening (warn when `$fields` keys collide with `$base` reserved keys in `api_log`/`_pb_write_api_log`), which addresses the class of bug even when review misses future instances.
