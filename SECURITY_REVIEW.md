...
#### 3. Client ID Validation is Permissive ✅ NOT APPLICABLE
**File:** [server/public/utils.php](server/public/utils.php#L45)

**Severity:** Medium (Marked as NOT A REAL RISK for this application)  
**Type:** Input Validation  
**Status:** CLOSED — Not applicable to this architecture

**Analysis:**
- ✅ Client IDs are **generated by the extension**, not user input
- ✅ Generated via `crypto.randomUUID()` (browser API, deterministic format)
- ✅ Stored in `chrome.storage.local` (not modifiable by external sources)
- ✅ Sent with every request by the extension code itself
- ✅ UUIDs always contain hyphens, which the regex explicitly allows
- ✅ `safe_file_path()` already protects against path traversal in file operations

**Why This Review Flagged It:**
Generic security template assumed client_id might be user-controllable. In reality, you control client_id generation entirely on the extension side, making it inherently secure.

**Recommendation:** Keep current implementation. No action needed. This is good defensive programming by design, not a vulnerability.
...
**Status:** Fixed in [security/implement-rate-limiting](https://github.com/jeffosness/pb-extension-dev/tree/security/implement-rate-limiting)

**Previous Risk:**
- `/api/core/sse_usage_stats.php` could be called unlimited times
- `/api/core/track_crm_usage.php` could flood the metrics logs
- No per-client rate limiting implemented

**Fixed Implementation:**
- Added `rate_limit_check()` function to [utils.php](server/public/utils.php) with file-based per-client rolling window tracking
- Added `rate_limit_or_fail()` to enforce limits with `429 Too Many Requests` response
- Added `cleanup_rate_limit_cache()` for periodic stale cache file cleanup
- **sse_usage_stats.php:** 60 requests/min per client_id (1 per second avg)
- **track_crm_usage.php:** 300 requests/min per client_id (5 per second avg)

**Implementation Details:**
```php
// Rate limiting functions in utils.php
...
rate_limit_or_fail($client_id, 60);  // or 300 for high-volume endpoints
```

**Storage:** File-based cache in `server/public/cache/rl_[client_id_hash].txt` with 60-second rolling window  
**Headers:** Returns `Retry-After: 60` on rate limit exceeded  
**Cleanup:** Automatic removal of cache files older than 1 hour

**Risk Mitigated:** Attackers can no longer flood metrics endpoints to block legitimate logging. Normal extension usage (1-5 requests/min) stays well under limits.
...
### Before Production:
- [x] **MEDIUM:** Client ID validation — ✅ NOT APPLICABLE (extension-generated, inherently secure)
- [ ] **LOW:** Add future-date validation (Issue #6)
- [x] **LOW:** Add rate limiting — ✅ DONE ([PR](https://github.com/jeffosness/pb-extension-dev/tree/security/implement-rate-limiting))
