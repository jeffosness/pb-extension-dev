...
- **[SUPPORT_RUNBOOK.md](SUPPORT_RUNBOOK.md)** — how to diagnose a customer report from a Support ID (or a client_id_hash + timestamp). Open this the moment a ticket lands — the exact SSH + grep + jq commands live there, plus the "what does event X mean" table. Saves you from re-figuring out the process months from now.
...
- **Tier 1** (extension code, popup UI, most api/ endpoints, softphone.php): requires a filled-in "Adversarial Review" section in the PR body. CI enforces the section's presence. Running the full [ADVERSARIAL_REVIEW_PLAYBOOK](ADVERSARIAL_REVIEW_PLAYBOOK.md) is encouraged for non-trivial changes but not required.
- **Tier 2** (security-critical: `utils.php`, `bootstrap.php`, webhooks, `sse.php`, `config.sample.php`, OAuth endpoints, `*_call_logger.php`, `SECURITY.md`): requires a Security Impact declaration (see SECURITY.md) AND execution of the full [ADVERSARIAL_REVIEW_PLAYBOOK](ADVERSARIAL_REVIEW_PLAYBOOK.md) — spawn multiple parallel hostile reviewers with diverse lenses, **run at least one cross-family review pass** (e.g., `codex review --base main` — 2 minutes, catches what same-family Claude subagents share blind spots on; LESSONS.md 2026-09-23 documents the specific BLOCKER this rule was added to prevent), adjudicate against the live branch, resolve every BLOCKER and MAJOR before merge. The PR-body "Adversarial Review" section for Tier 2 should paste the adjudicated findings table (or link the review artifacts), not just self-review talking points. See LESSONS.md 2026-07-27 for why "fill in a section" alone is insufficient: on that PR self-review missed a BLOCKER two independent hostile lenses caught. The 4-hour cool-off is enforced at **production deploy time** (`prod-*` tag push), NOT at PR merge. Merging to main auto-deploys to dev for soak-testing — that's when the cool-off clock starts. Cut the prod tag once the freshest Tier-2 commit has been on dev for 4 hours. Emergency override: cut the prod tag with a `-hotfix`, `-urgent`, or `-rollback` suffix (e.g. `prod-v0.8.2-hotfix`).
