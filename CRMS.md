...
- **`chrome-extension/STORE_LISTING.md`** — name the new CRM in the "Works With Your CRM" paragraph (prose), and add a short per-CRM feature section if it's L3. **Do NOT re-introduce a boilerplate-repeated bulleted list** ("X — Advanced Level 3 integration (API-based)" for every CRM) — that pattern got the v0.8.7 draft rejected by the Chrome Web Store as "excessive keywords" (see LESSONS.md 2026-08-25). The `crm-discoverability-check` CI only needs the displayName string to appear *somewhere* in the file; it does not need a per-CRM keyword row.
